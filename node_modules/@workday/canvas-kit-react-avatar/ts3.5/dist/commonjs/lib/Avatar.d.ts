import React from 'react';
import { SystemIconCircleSize } from '@workday/canvas-kit-react-icon';
export declare enum AvatarVariant {
    Light = 0,
    Dark = 1
}
export interface AvatarProps extends React.ButtonHTMLAttributes<HTMLButtonElement> {
    variant?: AvatarVariant;
    size?: SystemIconCircleSize | number;
    altText?: string;
    url?: string;
    as?: 'div';
}
declare type AvatarDivProps = Pick<AvatarProps, Exclude<keyof AvatarProps, keyof React.ButtonHTMLAttributes<HTMLButtonElement>>> & React.HTMLAttributes<HTMLDivElement>;
declare type AvatarOverload = {
    (props: {
        as: 'div';
    } & AvatarDivProps & {
        ref?: React.Ref<HTMLElement>;
    }): React.ReactElement;
    (props: Pick<AvatarProps, Exclude<keyof AvatarProps, 'as'>> & {
        ref?: React.Ref<HTMLButtonElement>;
    }): React.ReactElement;
    Variant: typeof AvatarVariant;
    Size: typeof SystemIconCircleSize;
};
declare const Avatar: AvatarOverload;
export default Avatar;
//# sourceMappingURL=Avatar.d.ts.map
